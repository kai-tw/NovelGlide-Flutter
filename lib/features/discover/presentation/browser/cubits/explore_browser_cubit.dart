...
import 'explore_browser_state.dart';

class ExploreBrowserCubit extends Cubit<ExploreBrowserState> {
  ExploreBrowserCubit(
  ...
  /// Browser use cases
  final ExploreBrowseCatalogUseCase _browseCatalogUseCase;

  /// Favorite use cases
  final ExploreGetFavoriteIdentifierByUriUseCase
      _getFavoriteIdentifierByUriUseCase;
  final ExploreRemoveFromFavoriteListUseCase _removeFromFavoriteListUseCase;
  ...
  Future<void> _loadFeed(Uri uri) async {
    ...
    if (!isClosed) {
      if (feed == null) {
        emit(const ExploreBrowserState(
          code: LoadingStateCode.error,
        ));
      } else {
        final ExploreBrowserState newState = state
            .copyWith(

  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ReaderSearchCubit, ReaderSearchState>(
      buildWhen: (previous, current) =>
          previous.code != current.code || previous.query != current.query,
      builder: (context, state) {
        final bool isDisabled =
            state.code == LoadingStateCode.loading || state.query.isEmpty;
        return IconButton(
          onPressed: isDisabled ? null : () => cubit.startSearch(),
          style: IconButton.styleFrom(
            backgroundColor:
                isDisabled ? null : Theme.of(context).colorScheme.primary,
            foregroundColor:
                isDisabled ? null : Theme.of(context).colorScheme.onPrimary,
          ),
          icon: Icon(
            Icons.search,

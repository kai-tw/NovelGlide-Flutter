...
class CollectionBackupRepositoryImpl implements CollectionBackupRepository {
  CollectionBackupRepositoryImpl(this._jsonPathProvider);
  ...
  Future<bool> deleteFromCloud() async {
    ...
    try {
      ...
      LogSystem.error(
        'Delete collection backup from Google Drive failed.',
        error: e,
        stackTrace: s,
      );
      return false;
  ...
  Future<String?> downloadFromCloud(
    ...
    try {
      ...
      LogSystem.error(
        'Download collection backup from Google Drive failed.',
        error: e,
        stackTrace: s,
      );
      return null;
  ...
  Future<bool> uploadToCloud(
    ...
    try {
      await GoogleApiInterfaces.drive.uploadFile(
        File(await _jsonPathProvider.collectionJsonPath),
        onUpload: onUpload,
      );
    } catch (e, s) {
      LogSystem.error(
        'Failed to upload collection backup to Google Drive.',
        error: e,
        stackTrace: s,
      );
      return false;

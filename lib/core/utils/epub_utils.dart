...
import '../../features/book_service/data/model/chapter_data.dart';
...
class EpubUtils {
  ...
  static Future<epub.EpubBook> loadEpubBook(String filePath) async {
    ...
    return await compute<Map<String, dynamic>, epub.EpubBook>(_loadEpubBookIsolate, <String, dynamic>{
  ...
  static Future<List<ChapterData>> getChapterList(String filePath) async {
    ...
    return epubBook.Chapters?.map((epub.EpubChapter e) => ChapterData.fromEpubChapter(e)).toList() ?? <ChapterData>[];
  }
  ...
  static epub.Image? findCoverImage(epub.EpubBook epubBook) {
    ...
    if (epubBook.Schema?.Package?.Manifest != null) {
      ...
      final epub.EpubManifestItem? coverItem = manifest.Items!.firstWhereOrNull((epub.EpubManifestItem item) =>
          item.Href != null &&
